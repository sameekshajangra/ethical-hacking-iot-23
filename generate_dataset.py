...
def generate_synthetic_iot23_data(num_samples=10000):
    ...
    for _ in range(num_samples):
        # 30% Benign, 70% Malicious (typical of IoT-23 which is heavily malicious)
        ...
        if label == 'Benign':
            ...
            if detailed == 'Mirai':
                duration = np.random.exponential(scale=0.1)  # short connections
                orig_bytes = np.random.normal(loc=150, scale=20) # typical syn payload
                resp_bytes = 0
    ...
    print(f"Generated synthetic IoT-23 dataset with {num_samples} rows: iot23_sample.csv")
